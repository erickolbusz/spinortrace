...
- <u>Gterms (optional):</u> a list of tuples `(vector, (index1, index2))` representing gluon propagators (e.g. $G_{\mu\nu}(l)$) contracted with the trace. 
...
```math
G(l)_{--} = \frac{l_\perp^2}{(l^-)^2}\ ,\ G(l)_{-\perp a} = \frac{l_{\perp a}}{l^-}\ ,\ G(l)_{\perp a\perp b} = -g_{\perp a\perp b}\ .
```
